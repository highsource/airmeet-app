...
import org.hisrc.airmeet.AirmeetPreferences;
...
public abstract class AbstractToggleTopicSubscribtionTask extends AsyncTask<String, Void, String[]> {
    ...
    protected String[] doInBackground(String... topics) {
        ...
        try {
            ...
            final String token = sharedPreferences.getString(AirmeetPreferences.TOKEN, null);
            if (token != null) {
    ...
    protected void onPostExecute(String[] topics) {
        ...
        if (topics != null) {
            SharedPreferences preferences =
                    PreferenceManager.getDefaultSharedPreferences(getContext());
            final Set<String> subscribedTopics = new TreeSet<String>(preferences.getStringSet(AirmeetPreferences.SUBSCRIBED_TOPICS, Collections.<String>emptySet()));
            for (String topic : topics)
            {
